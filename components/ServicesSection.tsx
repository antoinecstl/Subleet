...
function ServiceCard({ icon, title, desc, action, href, delay }: { icon: string; title: string; desc: string; action: string; href: string; delay: number }) {
    ...
    <AnimatedSection delay={delay}>
      <Link
        ...
        style={{
          ...
          cursor: 'pointer', display: 'block', textDecoration: 'none', height: '100%',
        }}
      >
        <div style={{
...
export default function ServicesSection() {
  return (
    <section style={{ padding: '120px clamp(20px, 5vw, 80px)', position: 'relative' }}>
