...
interface ApiKeyRegenerateProps {
  projectId: string;
  onKeyRegenerated?: () => void;
}

export default function ApiKeyRegenerate({ projectId, onKeyRegenerated }: ApiKeyRegenerateProps) {
  ...
  const handleConfirmRegenerate = async () => {
    ...
    try {
      const response = await fetch('/api/public/projects/api-key/regenerate', {
        method: 'POST',
        headers: {
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs sm:text-sm text-[var(--warning)]">
              ...
              <ul className="list-disc list-inside space-y-1 text-muted">
                ...
                <li>Your old API key is now invalid and cannot be used</li>
                <li>Update any applications using the old key immediately</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div className="glass-card rounded-xl p-4 sm:p-6">
        ...
        <div className="text-muted mb-6">
          <p className="mb-3 text-sm sm:text-base">
            <strong>Are you sure you want to regenerate your API key?</strong>
          </p>
          <p className="mb-2 text-sm sm:text-base">This action will:</p>
          <ul className="list-disc list-inside space-y-1 ml-4 text-sm sm:text-base">
            <li>Generate a completely new API key</li>
            <li>Invalidate your current API key immediately</li>
            <li>Update your chatbot&apos;s Edge Function automatically</li>
            <li>Require you to update any applications using the old key</li>
          </ul>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div className="flex-1">
          ...
          <p className="text-xs sm:text-sm text-muted">
            Lost your API key? You can regenerate a new one here.
          </p>
        </div>
        <button
